...
"use strict";	
	...
	$.fmatter.util = {
		...
		DateFormat : function (format, date, newformat, opts)  {
			...
			timezone = /\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC|CDT)(?:[-+]\d{4})?)\b/g,
			timezoneClip = /[^-+\dA-Z]/g,
	...
	$.fn.fmatter.date = function (cellval, opts, rwd, act) {
		...
		//strip "CDT", jqGrid does not support this as a timezone
			//Strip the first 4 characters, AKA D tag (3 letter representation of Day). jqGrid's regular expression to convert date between formats is wrong, it splices on T or Th for some reason, any date with Thu (Thursday) errors
			//and returns NaN		
			var re = /\s+CDT/g;
			var result = cellval.replace(re, "").substring(4);
			var temp0 = $.fmatter.util.DateFormat(op.srcformat, result,op.newformat,op);
			return temp0;
		} else {
			return $.fn.fmatter.defaultFormat(cellval, opts);
		}
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#"+fid+"_search").bind('click', function(){
					...
					// special hack
					if( filters.rules[0].data.length < 1 )
					{
						var elem = $("td.data").children("input:first");
						var actualValue = elem.val();
						filters.rules[0].data = actualValue;
					}
					// /special hack
					if(p.errorcheck) {
						fl[0].hideError();
